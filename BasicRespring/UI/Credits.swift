...
class CreditsViewController: UIViewController {
    ...
    //gradient
    @IBAction func CVE(_ sender: Any) {
        if let url = URL(string: "https://blogs.securiteam.com/index.php/archives/3796") {
            UIApplication.shared.open(url, options: [:])
        }
    }
    
    @IBAction func Twitter(_ sender: Any) {
        if let url = URL(string: "https://twitter.com/A_MrBenMitchell") {
            UIApplication.shared.open(url, options: [:])
        }
    }
}
